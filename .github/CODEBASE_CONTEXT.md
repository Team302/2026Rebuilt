...
AutonomousInit()
  ...
  └── AutonPreviewer::SetAuton() → sets the selected autonomous mode
  └── RobotState::SetState() → sets the robot state to AUTONOMOUS
  └── DragonField::UpdateEnabledStates()

TeleopInit()
  └── RobotState::SetState() → sets the robot state to TELEOP
  └── DragonField::UpdateEnabledStates()

TestInit()
  └── RobotState::SetState() → sets the robot state to TEST
  └── DragonField::UpdateEnabledStates()
```
...
- **Singletons**: 
  - `RobotState`: Manages robot state and events.
  - `MechanismConfigMgr`: Selects mechanism configurations based on robot type.
  - `DragonDataLoggerMgr`: Handles logging of telemetry data.
  - `FMSData`: Manages FMS-related data.

- **Initialization Order**:
  1. `RobotState`
  2. `MechanismConfigMgr`
  3. `DragonDataLoggerMgr`
  4. `FMSData`
...
- **Chassis Types**: Swerve drive configuration for 2026 season.
- **Commands**: Includes commands for driving, trajectory following, and season-specific commands.
...
- **Mechanism Types**: 
  - `Intake`: Handles game piece intake and expulsion.
  - `Launcher`: Responsible for launching game pieces.
  - `Climber`: Manages climbing mechanisms.

- **Intake Mechanism**:
  - **States**: 
    - `ExpelState`: State for expelling game pieces.
    - `ForceIntakeAutonState`: State for autonomous intake.
    - `IntakeState`: General state for intake operations.

---

## 7. State Machine Architecture

- **State Machine**: Each mechanism has its own state machine that manages its states and transitions.
- **Base Class**: `State` is the base class for all states, while `StateMgr` manages the state transitions.

---

## 8. Teleop Control System ⚠️ **Season-dependent**

- **Control Mapping**: Gamepad controls are mapped to robot functions.
- **Teleop Control Functions**: Includes functions for driving, manipulating mechanisms, and other robot operations.

---

## 9. Autonomous System ⚠️ **Season-dependent**

- **Autonomous Modes**: Defined in `CyclePrimitives` and selected via `AutonSelector`.
- **Autonomous Commands**: Includes commands for driving and executing specific tasks during autonomous mode.
...
- **Event System**: Uses a publish/subscribe model to notify subsystems of state changes.

---

## 11. Vision System

- **Components**: Integrates with Limelight and Quest for vision processing.
- **Pose Estimation**: Fuses vision data with odometry for accurate robot positioning.

---

## 12. Field Data & Target Calculation ⚠️ **Season-dependent**

- **Field Constants**: Contains poses for 2026 field elements.
- **Target Calculation**: Calculates target positions based on field data.

---

## 13. Logging & Telemetry

- **Telemetry**: Logs data for performance analysis and debugging.
- **Data Logger**: Manages logging of various telemetry data points.

---

## 14. Feedback & Dashboard

- **Driver Feedback**: Provides feedback to the drive team via dashboard HUD and LEDs.
- **Game Data Helper**: Parses game-specific messages from the FMS.

---

## 15. Utilities

- **Periodic Looper**: Manages periodic tasks for state machines.
- **Target Calculators**: Base and rebuilt target calculators for launcher targeting.

---

## 16. Robot Identifiers & Multi-Robot Support

- **Robot Identifiers**: Enum for identifying different robot configurations.
- **Multi-Robot Support**: Allows for different configurations based on the robot type.

---

## 17. Key Design Patterns

- **Singleton Pattern**: Used for managing shared resources and configurations.
- **State Pattern**: Used for managing the states of mechanisms and the robot.

---

## 18. Season-Dependent Change Checklist

- Ensure all commands and mechanisms are updated for the 2026 season.
- Verify that all autonomous modes are correctly implemented and tested.
- Update any field data and constants to reflect the 2026 game elements.
```
